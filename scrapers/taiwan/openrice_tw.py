# -*- coding: utf-8 -*-
"""
OpenRice 台灣爬虫 (API 分页模式)
https://tw.openrice.com/

台灣最大餐廳評價平台 (類似大眾點評)
通過內部 JSON API 獲取文章列表，無需 sitemap 或 HTML 解析
...
- 總計約 5880+ 篇文章，每頁最多 10 條

API 參數:
- page: 分頁 (1-based)
- cityId: 城市ID (1=台北)
- articleCategoryId: 分類ID (21=新店介紹, 22=餐廳消息, 24=美食精選, 25=專題企劃, 27=最新活動, 29=懶人包)
- sortType: 排序 (Default, PublishDateDesc, ViewCountDesc, IdDesc)
- filterType: 過濾 (Latest, Hot, Featured)
"""
...
class OpenRiceTWScraper(BaseScraper):
    """
    OpenRice 台灣爬虫
    ...
    CONFIG_KEY = "openrice_tw"

    # API 端點
    ARTICLES_API = "https://tw.openrice.com/api/articles"
    ...
    def __init__(self, use_proxy: bool = False):
        ...
        super().__init__(
            name=config.get("name", "OpenRice Taiwan"),
            base_url=config.get("base_url", "https://tw.openrice.com"),
        ...
        self.max_pages = config.get("max_pages", 600)
    ...
    def _parse_api_article(self, item: dict) -> Optional[Article]:
        ...
        if not url_path:
            return None
        url = f"https://tw.openrice.com{url_path}"
