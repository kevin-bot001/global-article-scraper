# -*- coding: utf-8 -*-
"""
${site_name} Scraper (Multi-City Sitemap Mode)
${base_url}
Multi-city support with city parameter
"""
import json
...
class ${class_name}(BaseScraper):
    ...
    Multi-city support: ${city_1}, ${city_2}, ${city_3}
    Usage: ${config_key}:${city_1}, ${config_key}:${city_2}

    Features:
    - Multi-city parameterization
    - City-specific base URL or sitemap
    - ${feature_1}
    """
    ...
    def __init__(self, city: str = None, use_proxy: bool = False):
        ...
        # Get country from cities config
        country = cities_config.get(self.city, "${default_country}")

        # City-specific base URL (if applicable)
        # Option 1: City in URL path
        base_url = f"{config.get('base_url', '${base_url}')}/{self.city}"
        # Option 2: City in subdomain
        # base_url = f"https://{self.city}.${domain}"
        ...
        super().__init__(
            ...
            use_proxy=use_proxy,
            country=country,
            default_city=self.city.replace("-", " ").title(),
        )
    ...
    def fetch_urls_from_sitemap(self) -> List[tuple]:
        """Fetch article URLs from city-specific sitemaps"""
        all_urls = []
    ...
    def is_valid_article_url(self, url: str) -> bool:
        # Ensure URL belongs to the correct city
        if not url or f"${domain}/{self.city}" not in url:
            return False
        ...
        # Require minimum path depth (city/slug)
...
# ============================================================
# config.py configuration for multi-city scraper:
# ============================================================
#
# "${config_key}": {
#     "name": "${site_name}",
#     "base_url": "${base_url}",
#     "delay": 0.5,
#     "country": "",  # Leave empty, determined by city
#     "default_city": "",
#     "cities": {
#         "${city_1}": "${country_1}",
#         "${city_2}": "${country_2}",
...
#
# ============================================================
# scrapers/__init__.py registration:
# ============================================================
#
# PARAMETERIZED_SCRAPERS = {
#     ...
#     "${config_key}": "city",  # ${config_key}:${city_1}, ${config_key}:${city_2}
# }
